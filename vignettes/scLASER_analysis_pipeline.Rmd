...
```{r}
cell_types  <- LETTERS[1:(2 + 1)]  # A-G (5 major + 2 minor)
time_points <- 3                   # V0, V1, V2
...
E_prog["A", ] <- c(+0.20, -0.10, +0.10)  
E_prog["B", ] <- c(-0.20, -0.10, -0.10)  
E_prog["G", ] <- c(+0.20, -0.10, +0.10)  
...
```{r}
obj3@metadata$disease = as.numeric(obj3@metadata$disease)
...
plot_top_nam_pc_umap(obj3, facet_by = NULL)
